...
export const DEFAULTS = {
  ...
  curtailRate: 0, // 출력제어율: 계통 제약으로 발전해도 못 파는 비율 (0~1)
 ...
 * @param {number} [input.curtailRate] - 출력제어율 (0~1)
...
export function calculateSolar(input) {
  ...
  const curtailRate = Math.min(1, num(input.curtailRate, DEFAULTS.curtailRate));
  ...
  // 1) 연간 이론 발전량 (kWh) = 설비용량 × 일평균 발전시간 × 365일 × 성능비
  const annualKwh = capacityKw * peakSunHours * 365 * performanceRatio;
  const annualMwh = annualKwh / 1000;

  // 1-1) 출력제어 반영: 판매 가능 발전량 = 이론 발전량 × (1 - 출력제어율)
  //      계통 제약으로 제어된 전력은 판매도 REC 발급도 되지 않는다.
  const soldKwh = annualKwh * (1 - curtailRate);
  const soldMwh = soldKwh / 1000;
  const curtailLossKwh = annualKwh - soldKwh;

  // 2) 연간 수익 = SMP(전력 판매) + REC(신재생 공급인증서) — 판매 가능 발전량 기준
  const smpRevenue = soldKwh * smpPrice;
  const recRevenue = soldMwh * recWeight * recPrice;
  const annualRevenue = smpRevenue + recRevenue;

  // 3) 환경 효과: 실제 계통에 공급된(판매된) 발전량 기준 CO2 절감량
  const co2ReductionKg = soldKwh * CO2_FACTOR;
